...
export function reportPost(targetId, targetType, parentType = "", parentId = "") {
  ...
  const reasonSelect = createElement(
    "select",
    { id: "report-reason" },
    REPORT_REASONS.map(opt => createElement("option", { value: opt.value }, [opt.label]))
  );

  const notesLabel = createElement("label", { for: "report-notes" }, ["Notes (optional):"]);
  const notesTextarea = createElement(
    "textarea",
    { id: "report-notes", rows: "4", placeholder: "Add any details (optional)…" },
    []
  );
  ...
  content.append(reasonLabel, reasonSelect, notesLabel, notesTextarea, messageP, submitBtn, cancelBtn);
  ...
  submitBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (response && response.reportId) {
        ...
        closeModal();
        Notify("Thanks. We'll review this shortly.", {
          type: "success",
          duration: 3000,
          dismissible: true,
        });
...
export function appealContent(targetId, targetType) {
  ...
  const infoP = createElement("p", {}, [
    "You're appealing the removal of the selected content. Provide a brief explanation below."
  ]);

  const reasonLabel = createElement("label", { for: "appeal-reason" }, ["Appeal explanation (required):"]);
  const reasonTextarea = createElement(
    "textarea",
    { id: "appeal-reason", rows: "5", placeholder: "Explain why this content should be restored…" },
    []
  );
  ...
  submitBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (res && res.appealId) {
        closeModal();
        Notify("Appeal submitted. A moderator will review it.", {
          type: "success",
          duration: 4000,
          dismissible: true,
        });
