...
//   /* ---------- Transform & Minimap Update ---------- */
  
//   /**
//    * Update map transform and minimap viewport
//    */
//   export function updateTransformAll(state, mapOptions, mapWrapper, minimap, minimapViewport) {
//     if (!mapWrapper || !mapOptions) return;
...
//     // Clamp map position so it doesn't move outside boundaries
//     const minX = viewportWidth - mapWidth * zoom;
//     const minY = viewportHeight - mapHeight * zoom;
//     state.mapX = clamp(state.mapX, Math.min(minX, 0), 0);
//     state.mapY = clamp(state.mapY, Math.min(minY, 0), 0);
//     // state.mapX = clamp(state.mapX, state.viewportWidth - mapOptions.mapWidth * state.zoom, 0);
//     // state.mapY = clamp(state.mapY, state.viewportHeight - mapOptions.mapHeight * state.zoom, 0);
...
//   /**
//    * Safely add an event listener and track it for later removal
//    * @param {object} state - shared map state (stores all listeners)
//    * @param {EventTarget} target - target element
//    * @param {string} event - event type
//    * @param {Function} fn - listener function
//    * @param {object|boolean} [opts={}] - options or capture flag
...
//     const hasOpts = opts && (opts.capture || opts.passive || opts.once);
//     const useOptions = isBool ? opts : (hasOpts ? opts : false);
//     const capture = !!(opts && opts.capture);
  
//     target.addEventListener(event, fn, useOptions);
//     if (!Array.isArray(state.listeners)) state.listeners = [];
//     state.listeners.push({ target, event, fn, capture });
//   }
...
//     for (const { target, event, fn, capture } of state.listeners) {
//       try {
