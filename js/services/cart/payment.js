...
//   // Calculate totals
//   let subtotal = 0;
//   Object.values(sessionData.items).flat().forEach(({ price, quantity }) => subtotal += price * quantity);

//   const discount = sessionData.couponCode && sessionData.couponCode.toLowerCase() === "save10"
...
//   const ul = createElement("ul", {});
//   Object.entries(sessionData.items).flatMap(([cat, items]) =>
//     items.forEach(({ itemName, quantity, unit, entityName, price }) => {
//       const lineTotal = price * quantity;
//       ul.appendChild(createElement("li", {}, [
//         `${itemName} – ${quantity} ${unit || ""}${entityName ? ` from ${entityName}` : ""} – `,
//         createElement("span", { class: "price" }, [`₹${lineTotal.toFixed(2)}`])
//       ]));
//     })
//   );
...
//   // Totals section
//   summary.appendChild(createElement("div", {}, [`Subtotal: ₹${subtotal.toFixed(2)}`]));
//   if (discount > 0) summary.appendChild(createElement("div", {}, [`Discount: −₹${discount.toFixed(2)} (${sessionData.couponCode})`]));
//   summary.appendChild(createElement("div", {}, [`Tax (5%): ₹${tax.toFixed(2)}`]));
//   summary.appendChild(createElement("div", {}, [`Delivery: ₹${delivery.toFixed(2)}`]));
//   summary.appendChild(createElement("p", { class: "total" }, [`Total Amount: ₹${total.toFixed(2)}`]));
...
//         paymentMethodUsed = paymentResult.method; // take actual method from modal
//       }

//       // Place order (fallback covers COD/UPI/Card directly)
//       confirmBtn.replaceChildren("Placing order...");
//       const payload = { ...sessionData, paymentMethod: paymentMethodUsed, total, discount };
...
// //   // Calculate totals including coupon if present
// //   let subtotal = 0;
// //   Object.values(sessionData.items).flat().forEach(({ price, quantity }) => subtotal += price * quantity);
...
// //     try {
// //       const method = document.getElementById("method").value;
// //       const payload = { ...sessionData, paymentMethod: method, total, discount };
