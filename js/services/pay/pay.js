...
export async function showPaymentModal({ entityType, entityId, entityName }) {
  ...
  if (entityType === "user") return { success: false };

  // --- UI Elements ---
  const errorBox = createElement("div", {
    class: "error-box",
    style: "display:none;color:red;font-size:0.9em;margin-top:6px;"
  });
  ...
  const paymentMethods = createElement("div", { class: "payment-methods" }, [
    createElement("div", { class: "balance-display" }, [
      `Wallet Balance: ${walletBalance.toFixed(2)} CCoin`
    ]),
    ...["wallet", "card", "upi", "cod"].map((method, i) =>
      createElement("label", {}, [
        createElement("input", {
          type: "radio",
          name: "payment-method",
  ...
  function setProcessing(state) {
    if (state) {
      ...
      confirmBtn.dataset.processing = "false";
      confirmBtn.textContent = "Confirm Payment";
    }
  }

  function validateInput(method, amount, currency) {
    if (!method) return "Please select a payment method.";
    if (!amount || amount <= 0) return "Please enter a valid amount.";
    if (method === "wallet" && currency !== "CCoin")
      return "Wallet payments are only supported in CCoin.";
    if (method === "wallet" && amount > walletBalance)
      return "Insufficient wallet balance.";
    return null;
  }

  // --- Payment Handlers Map ---
  const paymentHandlers = {
    async wallet({ amount, currency }) {
      const idempotencyKey = uuidv4();
      const res = await apiFetch(
        "/wallet/pay",
        "POST",
        { entityType, entityId, method: "wallet", amount, currency },
    ...
    async card({ amount, currency }) {
      // Placeholder for future integration
      return { success: true, message: "Card payment simulated." };
    },

    async upi({ amount, currency }) {
      // Placeholder for UPI deeplink / QR
      return { success: true, message: "UPI payment simulated." };
    },

    async cod() {
      // No transaction — pay on delivery
      return { success: true, message: "COD selected." };
    }
  };
    ...
    confirmBtn = Button(
      ...
      "confirm-payment-btn",
      {
        click: async () => {
          ...
          const validationError = validateInput(method, amount, currency);
          if (validationError) {
          ...
          setProcessing(true);
          try {
            const result = await handler({ amount, currency });
            if (result.success) {
              modal.close({ success: true, method, amount, currency });
            } else {
