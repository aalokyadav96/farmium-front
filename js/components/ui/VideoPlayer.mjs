...
//     r => typeof r === "number" && !isNaN(r)
//   );
//   const fallback = validQualities.length ? `${baseSrc}-${Math.min(...validQualities)}.mp4` : `${baseSrc}-360.mp4`;
//   if (qualityNum && validQualities.includes(qualityNum)) {
//     return `${baseSrc}-${qualityNum}.mp4`;
//   }
//   return fallback;
...
//   video.setAttribute("playsinline", "");
//   const baseSrc = src.replace(/\.(mp4|webm)$/, "");
//   const defaultSrc = resolutions?.length ? determineInitialSource(baseSrc, resolutions) : src;
//   video.src = defaultSrc;
//   video.poster = poster || `${baseSrc}.png`;
//   return video;
// };
...
//           else if (stopWhenOutOfView) video.pause();
//         });
//       },
//       { threshold: 0.5 }
//     );
...
//     const available = allQualities.filter(q => availableResolutions.includes(q));
//     const selector = createElement("select", { class: "quality-selector buttonx" },
//       available.map(q => createElement("option", { value: `${baseSrc}-${q}.mp4` }, [`${q}p`]))
//     );
...
//         poster,
//         theme,
//         qualities: (availableResolutions || []).map(q => ({
//           label: `${q}p`,
//           src: `${baseSrc}-${q}.mp4`
//         }))
//       });
...
// //   const userAutoPlay = localStorage.getItem("videoAutoPlay") === "true";
// //   const userAutoMute = localStorage.getItem("videoAutoMute") !== "false"; // default true
// //   const stopWhenOutOfView = localStorage.getItem("videoStopWhenOutOfView") === "true";
...
// //   // --- AutoPlay and Visibility Observer ---
// //   if (userAutoPlay) {
// //     const playWhenVisible = (entry) => {
// //       if (entry.isIntersecting) {
// //         video.play().catch(() => { });
