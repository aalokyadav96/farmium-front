...
export async function manageBandMembers(artistID, container) {
    ...
    const addBtn = Button("Add Member", "add-member-btn", { click: () => addBandMember(null, membersContainer) });
...
//     // Prepopulate socials if editing
//     if (mode === "edit" && existingArtist?.socials) {
//         Object.entries(existingArtist.socials).forEach(([platform, url]) => {
//             addSocialField({ platform, url }, socialsContainer);
//         });
...
//         if (artistid || name) {
//             const memberData = { role, dob };
//             if (artistid) memberData.artistid = artistid;
//             else memberData.name = name;
...
//         label: "Member Image (Not to be uploaded here)",
//         accept: "image/*"
//     });
...
//                 if (artist.image) {
//                     const imgSrc = resolveImagePath(EntityType.ARTIST, PictureType.THUMB, artist.image);
//                     let preview = memberDiv.querySelector(".member-image-preview");
//                     if (!preview) {
//                         preview = createElement("img", { class: "member-image-preview", style: "max-width:80px;display:block;margin-top:5px;" });
...
//     [idRow, nameField, roleField, dobField, imageField, removeBtn].forEach(el => memberDiv.appendChild(el));
//     container.appendChild(memberDiv);
// }
...
// //                 if (artist.image) {
// //                     const imageSrc = resolveImagePath(EntityType.ARTIST, PictureType.THUMB, artist.image);
// //                     let previewImg = memberDiv.querySelector(".member-image-preview");

// //                     if (!previewImg) {
// //                         previewImg = createElement("img", { class: "member-image-preview", style: "max-width:80px;display:block;margin-top:5px;" });
// //                         imageField.appendChild(previewImg);
// //                     }
