...
async function validateCoupon(couponCode, subtotal, entityId, entityType) {
...
export async function displayCheckout(container, passedItems = null) {
  ...
  try {
    ...
    renderAddressForm(container, async (address, couponCode) => {
      ...
      // derive entity info from first item
      const { entityId, entityType } = items[0] || {};
      const couponData = await validateCoupon(couponCode, subtotal, entityId, entityType);
      const { valid, discount } = couponData;
